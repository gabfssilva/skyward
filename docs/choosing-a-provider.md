...
Prices quoted below are what I was paying at the time of writing. Treat them as an order of magnitude, not a quote — run `sky offers list` against your own accounts for the current number.
...
Salad runs GPU containers on a distributed network of consumer machines. There is no private network between containers, so a Salad compute is always standalone: task dispatch works, distributed collections and distributed training do not. Its `priority` knob (`"high"` through `"batch"`) trades allocation latency against price.

Worth a look for embarrassingly parallel work — batch inference, sweeps — where each node is independent and price dominates.

### Lambda, Massed Compute, Novita, Scaleway, Vultr

All work with Skyward. I don't have strong opinions or specific experiences to share beyond that. They're solid options if their regions or pricing fit your needs — check `sky offers list` against your own accounts before assuming anything about their catalogs.
...
Spot mechanics vary by provider. AWS uses EC2 Fleet with a price-capacity-optimized allocation strategy — it picks instance pools where interruption is least likely. RunPod and Vast.ai use bid-based pricing where the multiplier directly controls how much you're willing to pay above the minimum. The strategies are different, but the principle is the same: a modest premium buys significantly more stability.

If a spot instance does get preempted, Skyward detects it automatically. The provider adapter reports the interruption, the reconciler notices the node is gone, and it provisions a replacement using the same offer. Your code doesn't need to handle this — but your workflow should be resilient to restarts.

### Checkpoints on object storage

The complement to spot instances: **save intermediate work somewhere that outlives the node**. If your instance gets preempted, you re-provision and resume from the last checkpoint instead of starting over. This turns spot interruption from a catastrophe into a minor inconvenience.
...
sky.Compute(
    ...
    allocation="spot",
    volumes=[sky.Volume(bucket="training-checkpoints", mount="/data", read_only=False)],
)
```

Volumes are read-only by default, since a mounted bucket is a network filesystem and most of what you do with one is read a dataset. A checkpoint directory is the exception — pass `read_only=False`.
...
- **Embarrassingly parallel, no inter-node traffic**: Salad
