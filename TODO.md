...
- [x] Paridade completa de `HealthChecker` — predicado Python remoto `Callable[[Info], bool | str]`, timeout por probe, atraso inicial e gate de readiness até o primeiro sucesso; falhas consecutivas reutilizam a substituição de node existente
- [x] Modo standalone `Options(cluster=False)` — cada node mantém runtime e cliente Casty independentes, sem bind distribuído nem coleções; dispatch normal (`>>`, `@`, `gather`) continua disponível. Providers sem rede privada usam standalone por padrão; `cluster=True` continua opt-in
...
- [x] Knobs específicos dos providers — factories públicas e adapters preservam as opções da v1 de provisioning, imagem, rede, storage e seleção de offers para AWS, GCP, Hyperstack, JarvisLabs, MassedCompute, Novita, TensorDock, VastAI, Verda, Scaleway, Vultr e Container. RunPod preserva os knobs que a REST v2 beta representa; `cluster_mode="instant"`, clock de CPU e mínimos de rede agora falham explicitamente porque o contrato v2 de pods GPU não expõe esses campos
- [x] Timeouts específicos por provider — `request_timeout` fica no cliente HTTP, `instance_timeout` arma shutdown remoto independente do TTL do compute, e polling/teardown permanecem nas fases próprias dos adapters
- [x] RunPod em REST v2 — `https://api.runpod.io/v2` para catálogo/offers, registry, datacenters, network volumes e lifecycle de pods; payloads e respostas cobertos por testes de contrato derivados do OpenAPI oficial. Secure/community e on-demand têm paridade. A v2 beta não publica criação ou preço spot, nem endpoints de Instant Cluster; offers spot não são anunciadas e pedidos explícitos falham, sem fallback silencioso para GraphQL/REST v1. A documentação pública ainda não informa a data de desligamento da REST v1
...
- [x] mTLS do cluster casty — a v1 emitia CA + certs de node e de cliente (`infra/tls.py`) e a v2 tinha perdido isso; não estava nesta lista. Restaurado com uma diferença: o CA é **por compute** e vive no banco (`computes.authority`), não em `~/.skyward/tls` do host — na v2 o control plane é o daemon e o compute sobrevive ao processo, então um CA local do host deixaria qualquer outro daemon sem acesso aos workers em execução. Compute bindado antes disso continua em claro
