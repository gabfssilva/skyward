...
from .state import _apply_bounds, _State
        ...
        async def receive(
            ...
            match msg:
                ...
                case BoundsChanged() as msg:
                    log.info(
                        "Bounds changed: min={min}, desired={desired}",
                        min=msg.min, desired=msg.desired,
                    )
                    return _maybe_scale(ctx, _apply_bounds(s, msg))
                ...
                case ReapIdleNodes(node_ids=ids, reason=reason):
                    if len(s.current) - len(ids) < s.min_nodes:
                        log.warning(
                            "Ignoring ReapIdleNodes: would violate min_nodes "
                            "(current={c}, reap={r}, min={m})",
                            c=len(s.current), r=len(ids), m=s.min_nodes,
                        )
                    ...
                    if new_failures >= max_provision_retries:
                        if len(s.current) >= s.min_nodes:
                            log.warning(
                                ...
                                n=new_failures, cur=len(s.current), min=s.min_nodes, err=error,
                            )
                            return watching(replace(new_s, consecutive_failures=0))
                        log.error(
