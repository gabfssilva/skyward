...
class PanelState:
    ...
    def _build_view_model(self, width: int, height: int, blink_on: bool) -> PanelViewModel:
        ...
        instance_vms = self._build_instance_vms(active_id, head_log_lines)
    ...
    def _build_infra_vm(self) -> InfraVM | None:
        ...
        # Build GPU info string: "8× A100-40GB (320 GB)"
        ...
        if self.infra.gpu_count and self.infra.gpu_model:
            ...
            if self.infra.gpu_vram_gb:
                gpu_info = f"{self.infra.gpu_count}× {self.infra.gpu_model}-{self.infra.gpu_vram_gb}GB ({vram_total} GB)"
            else:
                gpu_info = f"{self.infra.gpu_count}× {self.infra.gpu_model}"
    ...
    def _build_instance_vms(
        self, active_id: str | None, head_log_lines: int
    ) -> list[InstanceVM]:
