...
class InstanceRow:
    ...
    def render(self, width: int) -> list[Text]:
        ...
        # Log lines with | and L borders - only render actual logs
        ...
        for i in range(lines_to_render):
            ...
            log_line.append(border, style="bold")
    ...
    def _render_header(self) -> Text:
        ...
        # Status indicator (⏺ blinking during bootstrap, solid green when ready/done)
        match vm.status:
            case "bootstrapping":
                # Blink white during bootstrap
                if vm.blink_on:
                    line.append("⏺ ", style="white bold")
                else:
                    line.append("  ")
            case "ready":
                # Blink green during execution
                if vm.blink_on:
        ...
        # All metrics with sparklines (only show if we have data)
        metrics = [vm.cpu, vm.mem, vm.gpu, vm.gpu_mem, vm.temp]
        has_any_data = any(m.history for m in metrics)
        ...
        if has_any_data:
            ...
            line.append("  ")
            line.append_text(MetricBadge(vm.gpu_mem).render())
            line.append("  ")
