"""Deterministic pool fingerprinting.

Derives a human-readable slug from identity-defining fields (provider,
accelerator, region, image) so ``server``-mode pools can be matched
across script runs without the caller having to pick a name.
"""
...
def compute_fingerprint(spec: Spec) -> str:
    """Return a deterministic slug for ``spec``.
    ...
    ----------
    spec
        The user-facing :class:`~skyward.api.spec.Spec` to fingerprint.
