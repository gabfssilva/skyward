...
class FileController(Controller):
    ...
    async def upload(
        ...
        return dict(await files.put(compute_id, _target(node), path, await request.body()))
    ...
    async def download(
        ...
        return Stream(files.get(compute_id, _rank(node), path), media_type=BYTES)
