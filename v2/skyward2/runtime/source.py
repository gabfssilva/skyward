...
from skyward2.protocol.schemas import SkywardSource
from skyward2.runtime.journal import SKYWARD_DIR
...
async def resolve(mode: SkywardSource) -> Source:
    ...
    match detect() if mode == "auto" else mode:
        case "pypi":
            return Source(argument="skyward2")
        case "github":
...
def detect() -> Literal["local", "pypi"]:
    ...
    installed = packages_distributions().get("skyward2")
    if not installed:
        return "local"
...
def build() -> Path:
    ...
    out = tempfile.mkdtemp(prefix="skyward2-wheel-")
