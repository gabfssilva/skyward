...
from skyward2.runtime.source import Source, resolve
...
async def test_a_machine_becomes_a_worker(machine, key: asyncssh.SSHKey, source: Source):
    ...
    installed = await node._ssh.run("/opt/skyward/.venv/bin/python -c 'import msgspec, skyward2; print(skyward2.__name__)'")
    assert installed.stdout.strip() == "skyward2", "the image's packages and skyward itself are in the venv the worker runs in"
